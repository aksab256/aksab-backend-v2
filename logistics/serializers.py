...
class InventoryItemSerializer(serializers.ModelSerializer):
    ...
    product_code = serializers.ReadOnlyField(source='product.sku') # أضفنا الكود للبحث
...
# 2. سيريالايزر تفاصيل الأصناف داخل الإذن (الجديد)
class TransferItemSerializer(serializers.ModelSerializer):
    product_name = serializers.ReadOnlyField(source='product.name')
    unit = serializers.ReadOnlyField(source='product.unit')
    # لإظهار الصورة في تطبيق المندوب عند الاستلام
    product_image = serializers.SerializerMethodField()

    class Meta:
        model = TransferItem
        fields = ['id', 'product', 'product_name', 'product_image', 'quantity', 'unit_at_transfer', 'is_received']
