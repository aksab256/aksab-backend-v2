...
class StockTransfer(models.Model):
    ...
    # 🔥 السحر كله هنا: منطق تحريك المخزن عند التأكيد
    def save(self, *args, **kwargs):
        # 1. لو الحالة بقت COMPLETED، لازم ننفذ الحركة المخزنية
        if self.pk: # التأكد أن الإذن موجود مسبقاً وليس إنشاء جديد
            old_status = StockTransfer.objects.get(pk=self.pk).status
            if old_status != 'COMPLETED' and self.status == 'COMPLETED':
                with transaction.atomic():
                    for item in self.items.all():
                        # أ. خصم من مخزن المصدر (المصنع/الفرع)
                        sender_item, created = InventoryItem.objects.get_or_create(
                            warehouse=self.sender_warehouse,
                            product=item.product,
                            defaults={'stock_quantity': 0}
                        )
                        if sender_item.stock_quantity < item.quantity:
                            raise ValidationError(f"رصيد غير كافٍ للصنف {item.product.name} في مخزن المصدر")
                        
                        sender_item.stock_quantity -= item.quantity
                        sender_item.save()

                        # ب. إضافة لمخزن المندوب (المستلم)
                        receiver_item, created = InventoryItem.objects.get_or_create(
                            warehouse=self.receiver_warehouse,
                            product=item.product,
                            defaults={'stock_quantity': 0}
                        )
                        receiver_item.stock_quantity += item.quantity
                        receiver_item.save()
                        
                        # ج. تحديث حالة الصنف نفسه
